...
for i in range(len(pro)):
  ...
  activities = new_client.activity.filter(target_chemblid=[target['target_chembl_id'] for target in targets])
  for activity in activities:
     act=activity['standard_value']
     if act is not None:
        if int(float(str(act))) <=40000: #defined regarding to output of threshold function
